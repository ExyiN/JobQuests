...
public class ObjectiveTypeBreakStrategy implements ObjectiveType {
    ...
    public Component getDescription(int progression, int quantity) {
        String message = MessageFormat.format(jobQuests.getGuiConfig().getQuestGuiObjective().get(ObjectiveEventType.BREAK), progression, quantity, "<type>");
    ...
    public Component getCompletedMessage(int quantity) {
        String message = MessageFormat.format(jobQuests.getMessageConfig().getObjectiveBREAKCompleted(), quantity, "<type>");
        Map<String, Component> placeholders = new HashMap<>();
        placeholders.put("type", Component.translatable(type.translationKey()));
        return jobQuests.getMessageUtil().toMiniMessageComponent(message, placeholders);
    }
